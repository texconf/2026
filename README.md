...
講演タイトル、登壇者名、概要はMarkdownで管理します。pushでは`ci.yml`がビルド検証のみ行います。ローカル確認用に`npm run build`も使えます。
...
| `pages.yml` | 手動実行でビルド後に`gh-pages`へデプロイ（public化後にpushトリガーを足す） |
...
`pages.yml`はActionsタブから手動実行し、`gh-pages`ブランチへ公開用ファイルを書き出します。リポジトリをpublicにしたあと、pushトリガーを戻して自動デプロイにできます。
